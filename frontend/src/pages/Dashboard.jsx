...
import { clearToken, getCoverLetters, getProfile, getResumes, deleteResume, deleteCoverLetter } from '../utils/api'
      ...
      <nav className="bg-white border-b border-slate-200 shadow-sm sticky top-0 z-40">
        ...
        {menuOpen && (
          <div className="sm:hidden bg-white border-t border-slate-100 px-6 py-4 flex flex-col gap-3 text-sm">
            <button onClick={() => navigate('/home')} className="text-left hover:text-blue-700">Home</button>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div>
              ...
              <p className="text-blue-200 text-sm">
                Manage your resumes, cover letters, ATS scores and account from here.
              </p>
            </div>
        ...
        {activeTab === 'resumes' && (
          <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="text-lg font-bold text-slate-900">Saved Resumes</h2>
                <p className="text-sm text-slate-500">Edit, download or delete your resumes</p>
              </div>
              <button
                  ...
                  <tbody>
                    {resumes.map((resume) => (
                      <tr key={resume.id} className="border-b border-slate-100 hover:bg-slate-50 transition">
                        <td className="px-4 py-4 font-semibold text-slate-900">{resume.title}</td>
                        <td className="px-4 py-4 text-slate-500">{resume.updated_at || resume.created_at}</td>
                        <td className="px-4 py-4">
                          <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold
                            ${(resume.ats_score ?? 0) >= 70 ? 'bg-green-100 text-green-700' :
                              (resume.ats_score ?? 0) >= 40 ? 'bg-yellow-100 text-yellow-700' :
...
<button
  ...
  Delete
          ...
          <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-sm text-slate-500">Manage your tailored cover letters</p>
              </div>
              <button
            ...
            {coverLetters.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="font-semibold text-slate-700">No cover letters yet</p>
                <p className="text-sm text-slate-400 mt-1 mb-5">
              ...
              <div className="space-y-4">
                {coverLetters.map((letter) => (
                  <div key={letter.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <p className="font-semibold text-slate-900">{letter.job_title}</p>
                      <p className="text-sm text-slate-500">{letter.company_name} • {letter.created_at}</p>
                    </div>
                    <div className="flex gap-2">
                      <button className="px-3 py-1 text-xs font-semibold border border-slate-200 bg-white rounded-lg hover:bg-slate-100 transition">Edit</button>
                      <button className="px-3 py-1 text-xs font-semibold border border-slate-200 bg-white rounded-lg hover:bg-slate-100 transition">Download</button>
                      <button className="px-3 py-1 text-xs font-semibold border border-red-200 bg-red-50 text-red-700 rounded-lg hover:bg-red-100 transition">Delete</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === 'ats' && (
          <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <div className="mb-6">
              <h2 className="text-lg font-bold text-slate-900">ATS Score History</h2>
              <p className="text-sm text-slate-500">Track your resume performance over time</p>
            </div>

            {atsHistory.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                <div className="text-5xl mb-3">🎯</div>
                <p className="font-semibold text-slate-700">No ATS checks yet</p>
                <p className="text-sm text-slate-400 mt-1 mb-5">
              ...
              <div className="space-y-4">
                {atsHistory.map((entry) => (
                  <div key={entry.id} className="rounded-xl border border-slate-100 bg-slate-50 p-4 flex items-center justify-between gap-4">
                    <div>
                    ...
                    <span className={`px-4 py-2 rounded-full text-sm font-bold
                      ${entry.score >= 70 ? 'bg-green-100 text-green-700' :
                        entry.score >= 40 ? 'bg-yellow-100 text-yellow-700' :
                        'bg-red-100 text-red-700'}`}>
                      {entry.score}%
                    </span>
                  </div>
